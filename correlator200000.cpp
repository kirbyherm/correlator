...
int check_inputs(char* argv[]){
  ...
  if ((string(argv[2]) != "true") &&
      (string(argv[2]) != "rand_n")){
    std::cout << "Input error" << std::endl;
    std::cout << "Use true for true correlations or "
	      << "rand_n for random neutron correlations" << std::endl;
    return -1;
  }
...
//  Implantation dssd_implantation_pixels[40][40];
  ...
  for (int i=0; i<num; i++){
    ...
    punchthrough_event.scint_sub_event.Calibrate(scint_thresh); 
...
//         std::cout << temp_overflows << std::endl;
       ...
       implantation_event.scint_sub_event.Calibrate(scint_thresh);
...
*/
         ...
         if (dssd_implantation_pixels[implantation_event.dssd_sub_event.GetCStripLowFront()][implantation_event.dssd_sub_event.GetCStripLowBack()].empty() == false){
           ...
           implantation_event.SetTimeBetweenBackToBackImplantations(10000000000);
           dssd_implantation_pixels[implantation_event.dssd_sub_event.GetCStripLowFront()][implantation_event.dssd_sub_event.GetCStripLowBack()].push_back(implantation_event);
    ...
    decay_event.scint_sub_event.Calibrate(scint_thresh);
    ...
    if (decay_event.IsValidDecay(decays2)){
      ...
      int start_strip_front = front_lower_limit;
	  ...
	  if ((j >= 0) && (j <= 39) && (k >= 0) && (k<=39)){
	    if (dssd_implantation_pixels[j][k].empty()!= true){
            if (dssd_implantation_pixels[j][k].back().IsImplanted() == true ){ 
                PossImplants implantinfo; implantinfo.data[0] = j; implantinfo.data[1] = k; implantlist.push_back(implantinfo);
...
//               std::cout << " " << implant_count;
	         ...
	         //cout << "I managed to remove the first element" << endl;
               ...
               output_E_TAC_PIN1_I2S_raw = (*iter).tac_sub_event.Get_PIN1_I2S_Raw_Energy();
               output_E_TAC_PIN1_I2S_cal = (*iter).tac_sub_event.Get_PIN1_I2S_Cal_Energy();;
               
               output_E_TAC_PIN2_I2S_raw = (*iter).tac_sub_event.Get_PIN2_I2S_Raw_Energy();
               output_E_TAC_PIN2_I2S_cal = (*iter).tac_sub_event.Get_PIN2_I2S_Cal_Energy();
