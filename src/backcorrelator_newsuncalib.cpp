...
void SuN::GainMatchAndCalibrate(){
 ...
 // Bottom segments
  tSEG[0] = maxtime - (tPMT[0][1][1] + tPMT[0][1][2] + tPMT[0][1][3])/3.;
  tSEG[1] = maxtime - (tPMT[0][2][1] + tPMT[0][2][2] + tPMT[0][2][3])/3.;
  tSEG[2] = maxtime - (tPMT[0][3][1] + tPMT[0][3][2] + tPMT[0][3][3])/3.;
  tSEG[3] = maxtime - (tPMT[0][4][1] + tPMT[0][4][2] + tPMT[0][4][3])/3.;
  // Top Segments
  tSEG[4] = maxtime - (tPMT[1][1][1] + tPMT[1][1][2] + tPMT[1][1][3])/3.;
  tSEG[5] = maxtime - (tPMT[1][2][1] + tPMT[1][2][2] + tPMT[1][2][3])/3.;
  tSEG[6] = maxtime - (tPMT[1][3][1] + tPMT[1][3][2] + tPMT[1][3][3])/3.;
  tSEG[7] = maxtime - (tPMT[1][4][1] + tPMT[1][4][2] + tPMT[1][4][3])/3.;

  // Calibrate the SuN segments *** Updated Sept 2015 e14041 DR
  // Apply an individualized linear calibration to each segment

  // Bottom segments
  /*ecSEG[0] = 0.4303 * erSEG[0] - 3.8222;
  ecSEG[1] = 0.4343 * erSEG[1] - 4.0781;
  ecSEG[2] = 0.4242 * erSEG[2] - 3.5099;
  ecSEG[3] = 0.4303 * erSEG[3] - 6.0896;
  // Top segments
  ecSEG[4] = 0.4306 * erSEG[4] - 6.0403;
  ecSEG[5] = 0.4347 * erSEG[5] - 3.9827;
  ecSEG[6] = 0.4299 * erSEG[6] - 3.1289;
  ecSEG[7] = 0.4350 * erSEG[7] - 7.1781;
*/
...
bool Implantation::IsValidImplantation(){
  ...
  if (/*tac_sub_event.Get_I2S_I2N_Raw_Energy()  > 0 && tac_sub_event.Get_I2S_I2N_Raw_Energy()  < 32000 &&
...
int main(int argc, char* argv[]){
  ...
  */
  TFile* fOut = new TFile(Form("/mnt/analysis/e17028/correlated_rootfiles/%.0fms/bg/BWRun%i_SuN_%.0fms_%s_%ipx_bound.root",std::stod(argv[5])*1000,std::stoi(argv[1]),std::stod(argv[5])*1000,argv[7],(std::stoi(argv[3])*2-1)*(std::stoi(argv[3])*2-1)),"RECREATE");
...
//  Chain->Add(Form("/mnt/analysis/e14041/Ong/rootfiles/Run%s.root",argv[1]));
  ...
  */
  ...
  for (int i=num; i>0; i--){
    ...
    if ((isotope_cut->IsInside((corr_tof),(e_PIN2_cal)) && (e_PIN1_cal > 0) && cutDict["pin12"]->IsInside((e_PIN2_cal),(e_PIN1_cal)) && cutDict["i2pos"]->IsInside((E_TAC_I2S_I2N_raw),(E_PIN2_raw)))){
...
//	std::cout << "Decay!" << std::endl;
  ...
  if (std::string(argv[1]) == "2090" && std::string(argv[7]) == "ti57")
    bool copy = copyFile("/mnt/projects/e17028/hermansen/correlators/SuN-correlator/bg/backcorrelator_newsuncalib.cpp", Form("/mnt/analysis/e17028/correlated_rootfiles/%0.fms/backcorrelator_%s.cpp",std::stod(argv[5])*1000,currentDateTime().c_str()));
